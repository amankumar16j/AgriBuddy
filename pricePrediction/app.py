...
for commodity, csv_file in commodity_dict.items():
    commodity_list.append(Commodity(csv_file))
...
def TopFiveWinners():
    ...
    prev_month = current_month - 1 or 12  # Handle January case
    prev_year = current_year if prev_month != 12 else current_year - 1
    ...
    for i in commodity_list:
        ...
        prev_predict = i.getPredictedValue([float(prev_month), prev_year, prev_rainfall])
        ...
        if prev_predict != 0:
            ...
            change.append((0, commodity_list.index(i)))  # Handle appropriately

    
    sorted_change = change.copy()
    sorted_change.sort(reverse=True)
    to_send = []
    for j in range(0, 5):
        perc, idx = sorted_change[j]
        name = commodity_list[idx].getCropName()
        to_send.append([name.capitalize(), round((current_month_prediction[idx] * base[name.capitalize()]) / 100, 2), round(perc, 2)])
    return to_send



def TopFiveLosers():
    # Similar changes as in TopFiveWinners
    current_month = datetime.now().month
    current_year = datetime.now().year
    current_rainfall = annual_rainfall[current_month - 1]
    prev_month = current_month - 1 or 12  # Handle January case
